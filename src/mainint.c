...
#ifdef C4ALL
        if(!c4allMode){
        //RGG removed to keep more balanced for Comp4all version.
#endif
...
#ifdef C4ALL
        if (c4allMode) {
            printf("levelName: %s, lastLevelName %s",levelName,lastLevelName);
            if((strncmp(levelName, "ASSASSIN",8) == 0)||
               (strncmp(levelName, "GRYPHON",7) == 0)||
               (strncmp(levelName, "FLEET",5)==0)||//"New Deli",8) == 0)|| //not sure what happens there if the user do not take the secret level sphere...
               (strncmp(levelName, "NOSE DRIP",9) == 0)){
#ifdef GCW
                //GCW0 Comp4ALL Support
                sprintf(c4all_command, "opentyrian_ep_%dB",episodeNum);
                ...
                if (profile){
                    ...
                    spNetC4ACommitScore(profile,c4all_command,player[0].cash,&pointsScore,15000); //Timeout 15 Seconds
                    ...
                    switch(spNetC4AGetStatus()){
                        case SP_C4A_CANCELED:
                            printf("Online score uploading canceled!\n");
                        break;
                        case SP_C4A_TIMEOUT:
                            printf("Online score uploading resulted in a timeout\n");
                        break;
                        case SP_C4A_PROGRESS:
                            printf("Online score uploading did not finish.\n");
                        break;
                        case SP_C4A_OK:
                            printf("Online score uploading went fine.\n");
                        break;
                        case SP_C4A_ERROR:
                            printf("Online score uploading reported an error.\n");
                        break;
                    }
                }else{
                ...
                sprintf(c4all_command, "./sc so push opentyrian_ep_%dB pandora %lu >/dev/null 2>&1",episodeNum,player[0].cash);
                printf("c4all - Episode: %d - Score: %lu\n",episodeNum,player[0].cash);
                c4_all_ret = WEXITSTATUS(system(c4all_command));
...
#ifdef C4ALL
        JE_outTextGlow(VGAScreenSeg, 40, 110, tempStr);
#else
