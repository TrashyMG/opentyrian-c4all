...
bool select_gameplay( void )
{
	...
	for (; ; )
	{
		...
		{
			JE_outTextAdjust(VGAScreen, JE_fontCenter(gameplay_name[i], SMALL_FONT_SHAPES), i * 24 + 30, gameplay_name[i], 15, -4 + (i == gameplay ? 2 : 0) - (i == GAMEPLAY_NETWORK ? 4 : 0), SMALL_FONT_SHAPES, true);
		}
		JE_showVGA();
		...
		if (newkey)
		{
			...
			case SDLK_RETURN:
				...
				onePlayerAction = (gameplay == GAMEPLAY_ARCADE_1P);
				twoPlayerMode = (gameplay == GAMEPLAY_ARCADE_2P);
#ifdef C4ALL
				c4allMode = (gameplay == GAMEPLAY_COMPO4ALL);
				if(c4allMode){
...
#endif
        for (int i = diff_num; i <= difficulty_max; i++)
		{
